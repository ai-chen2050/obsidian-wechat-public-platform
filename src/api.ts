...
export default class ApiManager {
	...
	async newDraftToWechat(
		...
		try {
			...
			const MdImagedContent = await this.handleMDImage(
				filePath,
				content,
				"wx",
			);
			...
			const htmlText = await markedParse(
				MdImagedContent,
				parseOptions,
				[],
			);
	...
	async handleMDImage(
		...
		const promises = matches.map(async (match) => {
			...
			let responseUrl;
			if (to === "wx") {
				responseUrl = await this.uploadImageToWx(
					filePath,
					imagePath,
					"",
				);
			} else if (to === "bjh") {
				responseUrl = await this.uploadImageToBjh(
					filePath,
					imagePath,
					"",
				);
		...
		const promises2 = matches2.map(async (match) => {
			...
			const responseUrl = await this.uploadImageToWx(
				filePath,
				imgfile?.path!,
				"",
			);
	...
	async getUserCumulate(beginDate: string, endDate: string): Promise<any> {
		try {
			...
			if (pass === false) return undefined;
			const url = `https://api.weixin.qq.com/datacube/getusercumulate?access_token=${settings.accessToken}`;
			const req: RequestUrlParam = {
				url,
				method: "POST",
				headers: this.getHeaders(),
			...
			if (resp.json["errcode"]) {
				new Notice(
					`获取用户累计数据失败: ${resp.json["errcode"]} ${resp.json["errmsg"]}`,
				);
				return undefined;
			}
			return resp.json;
		} catch (e) {
			new Notice("获取用户累计数据失败");
			console.error("getUserCumulate error", e);
		}
	}

	async getBizSummary(beginDate: string, endDate: string): Promise<any> {
		try {
			const settings = get(settingsStore);
			const pass = await this.refreshAccessToken(
				settings.appid,
			...
			if (pass === false) return undefined;
			const url = `https://api.weixin.qq.com/datacube/getbizsummary?access_token=${settings.accessToken}`;
			const req: RequestUrlParam = {
				url,
				method: "POST",
				headers: this.getHeaders(),
			...
			if (resp.json["errcode"]) {
				new Notice(
					`获取发表内容概况失败: ${resp.json["errcode"]} ${resp.json["errmsg"]}`,
				);
				return undefined;
			}
			return resp.json;
		} catch (e) {
			new Notice("获取发表内容概况失败");
			console.error("getBizSummary error", e);
		}
	}
